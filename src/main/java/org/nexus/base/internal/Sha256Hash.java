 ...
 * {@link #hashCode} work correctly, allowing it to be used as a key in a map.
 * It also checks that the {@code length} is correct (equal to {@link #LENGTH})
 * and provides a bit more type safety.
 * <p>
 * Given that {@code Sha256Hash} instances can be created using
 * {@link #wrapReversed(byte[])} or {@link #twiceOf(byte[])} or by wrapping raw
 * bytes, there is no guarantee that if two {@code Sha256Hash} instances are
 * found equal (via {@link #equals(Object)}) that their preimages would be the
 * same (even in the absence of a hash collision.)
 */
public class Sha256Hash implements Comparable<Sha256Hash> {
     ...
     * @throws IllegalArgumentException if the given array length is not exactly
     * 32
     */
     ...
     * @return a new instance
     * @throws IllegalArgumentException if the given string is not a valid hex
     * string, or if it does not represent exactly 32 bytes
     */
     ...
     * @return a new instance
     * @throws BufferUnderflowException if the read hash extends beyond the
     ...
     * @return the buffer
     * @throws BufferOverflowException if the hash doesn't fit the remaining
     * buffer
     */
