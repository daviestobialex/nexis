...
import org.nexis.utilities.ContextPropagatingThreadFactory;
 ...
 * scoped to a specific instantiation of bitcoinj for a specific network. You
 * can get an instance of this class through calling {@link #get()}.</p>
 *
 * <p>
 * Context is new in 0.13 and the library is currently in a transitional period:
 * you should create a Context that wraps your chosen network parameters before
 * using the rest of the library. However if you don't, things will still work
 * as a Context will be created for you and stashed in thread local storage. The
 * context is then propagated between library created threads as needed. This
 * automagical propagation and creation is a temporary mechanism: one day it
 * will be removed to avoid confusing edge cases that could occur if the
 * developer does not fully understand it e.g. in the case where multiple
 * instances of the library are in use simultaneously.</p>
 */
public class Context {
     ...
     * framework. Eventually you will be expected to do this yourself in the
     * same manner as fetching a NetworkParameters object (at the start of your
     * app).
     */
    public Context() {
    ...
    /**
     * Creates a new custom context object. This is mainly meant for unit tests
     * for now.
     *
     * @param eventHorizon Number of blocks after which the library will delete
     * data and be unable to always process reorgs. See
     * {@link #getEventHorizon()}.
     * @param feePerKb The default fee per 1000 virtual bytes of transaction
     * data to pay when completing transactions. For details, see
     * {@link SendRequest#feePerKb}.
...
//        log.info("Creating bitcoinj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * thread</b>.BitcoinJ is an API that has thread affinity: much like OpenGL
     * it expects each thread that accesses it to have been configured with a
     * global Context object. This method returns that. Note that to help you
     * develop, this method will <i>also</i> propagate whichever context was
     * created last onto the current thread, if it's missing. However it will
     * print an error when doing so because propagation of contexts is meant to
     * be done manually: this is so two libraries or subsystems that
     * independently use bitcoinj (or possibly alt coin forks of it) can operate
     * correctly.
     *
     * @return
     * @throws java.lang.IllegalStateException if no context exists at all or if
     * we are in strict mode and there is no context.
     */
    public static Context get() {
        Context tls = slot.get();
        if (tls == null) {
            if (isStrictMode) {
                log.severe("Thread is missing a bitcoinj context.");
                log.severe("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
            ...
            log.severe("Performing thread fixup: you are accessing bitcoinj via a thread that has not had any context set on it.");
            log.severe("This error has been corrected for, but doing this makes your app less robust.");
            log.severe("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
            log.severe("Please refer to the user guide for more information about this.");
            log.severe("Thread name is " + Thread.currentThread().getName());
     ...
     * {@link ContextPropagatingThreadFactory}, rather than using a heuristic
    ...
    public static Context getOrCreate() {
        ...
        try {
            ...
            log.severe("Implicitly creating context. This is a migration step and this message will eventually go away.");
            context = new Context();
            return context;
        }
        return context;
    ...
    /**
     * Sets the given context as the current thread context. You should use this
     * if you create your own threads that want to create core BitcoinJ objects.
     * Generally, if a class can accept a Context in its constructor and might
     * be used (even indirectly) by a thread, you will want to call this first.
     * Your task may be simplified by using a
     ...
     * tracks advertised and downloaded transactions so their confidence can be
     * measured as a proportion of how many peers announced it. With an
     * un-tampered with internet connection, the more peers announce a
     * transaction the more confidence you can have that it's really valid.
     *
     * @return
     */
     ...
     * library consider a transaction to be so confirmed that it's safe to
     * delete data.Re-orgs larger than the event horizon will not be correctly
    ...
    /**
     * The default fee per 1000 virtual bytes of transaction data to pay when
     * completing transactions.For details, see {@link SendRequest#feePerKb}.
     *
     * @return
     */
    ...
    /**
     * If this is set to true, proof of work is not enforced.This is useful for
     * unit-testing, as we need to create and solve fake blocks quite often.
     *
     * @return
     */
