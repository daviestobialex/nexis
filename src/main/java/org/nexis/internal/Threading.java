...
import org.nexis.utilities.PlatformUtils;

/**
 * Various threading related utilities. Provides a wrapper around explicit lock
 * creation that lets you control whether bitcoinj performs cycle detection or
 * not. Cycle detection is useful to detect bugs but comes with a small cost.
 * Also provides a worker thread that is designed for event listeners to be
 * dispatched on.
 */
public class Threading {
    ...
    /**
     * A dummy executor that just invokes the runnable immediately. Use this
     * over more complex executors (e.g. those extending
     * {@link ExecutorService}), which are overkill for our needs.
     */
    public static final Executor SAME_THREAD;

    /**
     * Put a dummy task into the queue and wait for it to be run. Because it's
     * single threaded, this means all tasks submitted before this point are now
     * completed. Usually you won't want to use this method - it's a convenience
     * primarily used in unit testing. If you want to wait for an event to be
     * called the right thing to do is usually to create a
     * {@link CompletableFuture} and then call
     * {@link CompletableFuture#complete(Object)} on it. For example:
     * <pre>{@code
     * CompletableFuture f = CompletableFuture.supplyAsync(() -> event, USER_THREAD)
     * }</pre> You can then either block on that future, compose it, add
     * listeners to it and so on.
     */
    ...
    /**
     * An exception handler that will be invoked for any exceptions that occur
     * in the user thread, and any unhandled exceptions that are caught whilst
     * the framework is processing network traffic or doing other background
     * tasks. The purpose of this is to allow you to report back unanticipated
     * crashes from your users to a central collection center for analysis and
     * debugging. You should configure this <b>before</b> any bitcoinj library
     * code is run, setting it after you started network traffic and other forms
     * of processing may result in the change not taking effect.
     */
//    @Nullable
    ...
    public static class UserThread extends Thread implements Executor {
        ...
        private static final Logger log = Logger.getLogger(UserThread.class.getName());
        ...
        public void run() {
            while (true) {
                ...
                try {
                    ...
                    log.warning("Exception in user thread " + throwable.getMessage());
        ...
        public void execute(Runnable command) {
            ...
            if (size == WARNING_THRESHOLD) {
                log.log(Level.WARNING,
                        "User thread has {0} pending tasks, memory exhaustion may occur.\n"
