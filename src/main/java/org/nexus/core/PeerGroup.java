...
import static org.nexus.core.PeerRegistry.DEFAULT_MAX_CONNECTIONS;
import org.nexus.listeners.PeerConnectListener;
import org.nexus.messages.HandshakeRequestMessage;
...
public class PeerGroup {
    ...
    private void seed(int maxConnections) {
        ...
        ConcurrentMap<PublicNodeProperties, Channel> activePeers = peerRegistry.getActivePeers();

        for (String address : dnsSeeds) {
            if (activePeers.size() > maxConnections) {
                break;
            }
    ...
    public void beginMessagePropagation(NexusEnvelopBuilder builder) {
        ConcurrentMap<PublicNodeProperties, Channel> activePeers = peerRegistry.getActivePeers();
        ConcurrentMap<PublicNodeProperties, Channel> pendingPeers = peerRegistry.getPendingPeers();
...
//            ManifestRequestMessage manifestMessage = new ManifestRequestMessage(params, manifest, nodeId.getId());
            NexusProtocol.Handshake handshake = NexusProtocol.Handshake.newBuilder()
                    .setNonce(ThreadLocalRandom.current().nextLong())
                    .build();
